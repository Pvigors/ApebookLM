...
export default function ExperienceLogin({ accessKey }: { accessKey: string }) {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    setBusy(true);
    try {
      const response = await fetch(`/api/auth/experience/${encodeURIComponent(accessKey)}`, {
        method: "POST",
    ...
    >
      ...
      <section className="w-full max-w-[420px] rounded-[24px] bg-panel px-6 py-8 shadow-[0_24px_64px_-28px_rgb(42_34_91_/_0.48)] sm:px-8 sm:py-9">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-[17px] font-bold tracking-[.01em] text-ink">猿笔记</p>
            <p className="mt-0.5 text-[12px] text-muted">ApeNotes</p>
          </div>
        </div>

        <div className="mt-8">
          <h1 className="cjk-display text-[24px] font-bold text-ink">测试账号登录</h1>
          <p className="mt-2 text-[13px] leading-6 text-ink2">请输入分配给你的用户名和密码。</p>
        </div>

        <form className="mt-6 space-y-4" onSubmit={submit}>
          <label className="block">
            <span className="mb-1.5 block text-[12.5px] font-medium text-ink2">用户名</span>
            <input
              name="username"
              type="text"
